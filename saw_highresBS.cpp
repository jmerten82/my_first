...
int main(int argc, char *argv[])
{
  ...
  gsl_matrix *convmerge;
  gsl_matrix *lowconv;
  gsl_matrix *highconv;
  gsl_matrix *shear1merge;
  gsl_matrix *lowshear1;
  gsl_matrix *highshear1;
  gsl_matrix *shear2merge;
  gsl_matrix *lowshear2;
  gsl_matrix *highshear2;
  gsl_matrix *jacdetmerge;
  gsl_matrix *lowjacdet;
  gsl_matrix *highjacdet;


  //Performing the interpolation, cutting and masking of the initial field

  for(int i = 0; i <= options1.showint("bootstrap"); i++)
      {
	  ...
	  convmerge = gsl_matrix_calloc(options1.showint("finaly"),options1.showint("finalx"));
	  lowconv = gsl_matrix_calloc(oy_dim,ox_dim);
	  highconv = gsl_matrix_calloc(cuty,cutx);
	  shear1merge = gsl_matrix_calloc(options1.showint("finaly"),options1.showint("finalx"));
	  lowshear1 = gsl_matrix_calloc(oy_dim,ox_dim);
	  highshear1 = gsl_matrix_calloc(cuty,cutx);
	  shear2merge = gsl_matrix_calloc(options1.showint("finaly"),options1.showint("finalx"));
	  lowshear2 = gsl_matrix_calloc(oy_dim,ox_dim);
	  highshear2 = gsl_matrix_calloc(cuty,cutx);
	  jacdetmerge = gsl_matrix_calloc(options1.showint("finaly"),options1.showint("finalx"));
	  lowjacdet = gsl_matrix_calloc(oy_dim,ox_dim);
	  highjacdet = gsl_matrix_calloc(cuty,cutx);
	  ...
	  gsl_matrix_free(lowconv);
	  gsl_matrix_free(highconv);
	  gsl_matrix_free(shear1merge);
	  gsl_matrix_free(lowshear1);
	  gsl_matrix_free(highshear1);
	  gsl_matrix_free(shear2merge);
	  gsl_matrix_free(lowshear2);
	  gsl_matrix_free(highshear2);
	  gsl_matrix_free(jacdetmerge);
	  gsl_matrix_free(lowjacdet);
	  gsl_matrix_free(highjacdet);
