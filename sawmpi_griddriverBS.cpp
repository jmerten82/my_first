...

{
  ...
  for(int j = 0; j <= options1.showint("bootstrap"); j++)
      {
	  ...
	  if(my_rank == 0)
		  ...
		  if(j != 0)
		      {
			  if(options1.showbool("shear"))
			      {
				  CatReader ellipreader(options1.showstring("ellipinput"));
				  CatWriter ellipwriter(&ellipreader);
				  Bootstrap ellipBS(1,1,&ellipwriter,options1.show_filename("ellipinput",0));
				  ellipBS.run(j);
			      }
			  
			  if(options1.showbool("flexion"))
			      {
				  CatReader flexionreader(options1.showstring("flexioninput"));
				  CatWriter flexionwriter(&flexionreader);
				  Bootstrap flexionBS(1,1,&flexionwriter,options1.show_filename("flexioninput",0));
				  flexionBS.run(j);
			      }
			  
			  if(options1.showbool("ccurve"))
			      {
				  CatReader ccurvereader(options1.showstring("ccurveinput"));
				  CatWriter ccurvewriter(&ccurvereader);
				  Bootstrap ccurveBS(1,1,&ccurvewriter,options1.show_filename("ccurveinput",0));
				  ccurveBS.run_delta(j);
			      }
			  if(options1.showbool("msystems"))
			      {
				  throw invalid_argument("Bootstrapping of msystems not possible");
			      }
		      }
	      }
